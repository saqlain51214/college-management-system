    ...
    <form wire:submit.prevent="search"
        ...
        <div class="flex-1">
            <label class="mb-1 block text-xs font-medium uppercase tracking-wide text-gray-500">
                Teacher Name or Employee ID
            </label>
            <input type="text" wire:model="q" autofocus
                   placeholder="e.g. Ali Khan or EMP-0012"
    ...
    @if ($teacher)
        ...
        <div class="rounded-xl border border-gray-200 bg-white p-5 dark:border-white/10 dark:bg-gray-900">
            <div class="flex flex-wrap items-start justify-between gap-4">
                <div>
                    <h2 class="text-lg font-bold text-gray-900 dark:text-gray-50">{{ $teacher['name'] }}</h2>
                    <p class="text-sm text-gray-500">{{ $teacher['designation'] ?: '—' }}</p>
                </div>
                <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $teacher['active'] ? 'bg-green-100 text-green-700' : 'bg-gray-200 text-gray-600' }}">
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-3">
            @foreach ([
                ['Total Salary (Life-to-date)', $rs($totals['net'] ?? 0), '#6b7280'],
        ...
        <div class="rounded-xl border border-gray-200 bg-white dark:border-white/10 dark:bg-gray-900">
            <div class="flex items-center justify-between border-b border-gray-200 px-4 py-3 dark:border-white/10">
                <h3 class="text-sm font-bold text-gray-800 dark:text-gray-100">Salary History</h3>
                <span class="text-xs text-gray-500">{{ $totals['count'] ?? 0 }} total · {{ $totals['pending'] ?? 0 }} with balance</span>
            </div>
                    ...
                    <tbody>
                        @forelse ($payments as $row)
                            <tr class="border-b border-gray-100 dark:border-white/5">
                                <td class="px-4 py-2 font-mono text-xs text-gray-600 dark:text-gray-300">{{ $row['reference'] }}</td>
                                <td class="px-4 py-2 text-gray-700 dark:text-gray-200">{{ $row['period'] }}</td>
                                ...
                                <td class="px-4 py-2">
                                    <span class="rounded-full px-2 py-0.5 text-xs font-medium {{ $row['status'] === 'paid' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700' }}">
                                        {{ ucfirst($row['status']) }}
                                    </span>
                                </td>
                                <td class="px-4 py-2 text-gray-500">{{ $row['due'] }}</td>
                                <td class="px-4 py-2 text-gray-500">{{ $row['paid_on'] }}</td>
                                <td class="px-4 py-2 text-gray-500">{{ $row['method'] ? ucfirst(str_replace('_', ' ', $row['method'])) : '—' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="9" class="px-4 py-8 text-center text-gray-400">No salary records for this teacher yet.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
