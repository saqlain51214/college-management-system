...
<section class="relative overflow-hidden py-14 sm:py-20" style="background:var(--site-body-bg)">
    ...
    <div class="relative mx-auto max-w-6xl px-4 sm:px-6">
        ...
        @else
            {{-- ── Card layout: photo-forward, no overlap with text ── --}}
            <div class="grid gap-7 sm:gap-8 md:grid-cols-2 lg:grid-cols-3">
                @foreach($leaders as $L)
                    @php $initials = collect(explode(' ', trim($L->name)))->take(2)->map(fn ($w) => mb_substr($w, 0, 1))->implode(''); @endphp
                    <a href="{{ route('leadership.message', $L) }}" data-reveal data-reveal-delay="{{ $loop->index % 3 + 1 }}"
                       class="group flex flex-col overflow-hidden rounded-[26px] bg-white shadow-[0_10px_40px_-18px_rgba(0,0,0,0.18)] ring-1 ring-stone-100 transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.28)]">
                        {{-- Photo panel — text never overlaps the image --}}
                        <div class="relative aspect-[5/4] w-full overflow-hidden">
                            ...
                            <div class="absolute inset-x-0 bottom-0 p-5">
                                <h3 class="font-display text-lg font-bold text-white drop-shadow-sm">{{ $L->name }}</h3>
                                <p class="text-sm font-semibold" style="color:var(--site-gold)">{{ $L->designation }}</p>
                                @if($L->organization)
                                    <p class="text-xs text-white/70">{{ $L->organization }}</p>
                                @endif
                            </div>
                        </div>

                        <div class="flex flex-1 flex-col p-6">
                            <svg class="mb-2 h-6 w-6 opacity-20" fill="currentColor" viewBox="0 0 32 32" style="color:var(--site-brand)">
                                <path d="M10 8c-3.3 0-6 2.7-6 6v10h10V14H8c0-1.1.9-2 2-2V8zm16 0c-3.3 0-6 2.7-6 6v10h10V14h-6c0-1.1.9-2 2-2V8z"/>
                            </svg>
