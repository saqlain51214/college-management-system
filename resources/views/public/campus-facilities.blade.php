...
<div class="mx-auto max-w-6xl px-4 sm:px-6 py-12">
    @php
        $facilityIcons = [
            ...
            'admin'      => 'M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z',
            'sports'     => 'M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6',
            'prayer'     => 'M12 2a10 10 0 100 20A10 10 0 0012 2zm0 0v4m0 12v-4M4.22 4.22l2.83 2.83m9.9 9.9l2.83 2.83M2 12h4m12 0h4M4.22 19.78l2.83-2.83m9.9-9.9l2.83-2.83',
            'canteen'    => 'M3 3h2l.4 2M7 13h10l4-8H5.4M7 13L5.4 5M7 13l-2.293 2.293c-.63.63-.184 1.707.707 1.707H17m0 0a2 2 0 100 4 2 2 0 000-4zm-8 2a2 2 0 11-4 0 2 2 0 014 0z',
        ...
        $facilities = data_get($cmsPage ?? null, 'content.facilities') ?: \App\Models\WebsitePage::defaultContentFor('campus-facilities')['facilities'];
    @endphp
    <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div class="rounded-2xl border border-stone-200 bg-white p-6 hover:shadow-md transition-shadow">
            <div class="w-12 h-12 rounded-xl flex items-center justify-center mb-4 shadow-sm" style="background:var(--site-brand)">
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8" d="{{ $facilityIcons[$facility['icon'] ?? ''] ?? $facilityIcons['classrooms'] }}"/></svg>
            </div>
            <h3 class="font-bold text-stone-800 mb-2">{{ $facility['title'] ?? '' }}</h3>
            <p class="text-sm text-stone-600 leading-relaxed">{{ $facility['description'] ?? '' }}</p>
        </div>
        @endforeach
    </div>
