...
class PublicController extends Controller
{
    ...
    public function feeChallanLookup(Request $request)
    {
        ...
        return $this->renderFeeChallanLookup(trim($request->input('identifier')));
    }

    /**
     * Public self-service: generate a new fee challan (full remaining balance,
     * or a self-chosen partial amount) for the current semester's fee.
     * Re-verifies the reg/roll number (same guard as feeChallanPdf) since this
     * is an unauthenticated endpoint that now creates records — throttled via
     * the public-fee-challan limiter, and capped (3 unpaid installments) so a
     * student cannot pile up unlimited unpaid self-service slips. Re-renders
     * the same lookup results rather than redirecting, so the student isn't
     * forced to search twice.
     */
    public function feeChallanGenerateSlip(Request $request)
    {
        $request->validate([
            'identifier' => ['required', 'string', 'max:60'],
            'mode'       => ['required', 'in:full,partial'],
            'amount'     => ['required_if:mode,partial', 'nullable', 'numeric', 'min:1'],
        ]);
        ...
        if (! $student) {
            return $this->renderFeeChallanLookup($identifier, 'No student found for that registration/roll number.');
        }

        $feeType  = 'tuition';
        $semester = $student->current_semester;
        $invoice  = \App\Models\FeePayment::invoiceSummary($student, $feeType, $semester, $student->academic_year_id);
        $amount   = $request->input('mode') === 'full' ? $invoice['available'] : $request->input('amount');
        ...
        try {
            $slip = \App\Models\FeePayment::generateSlip($student, [
                ...
                'amount'           => $amount,
                'remarks'          => 'Self-generated by student via the public fee-challan page (' . $request->input('mode') . ').',
            ]);
        } catch (\InvalidArgumentException $e) {
            return $this->renderFeeChallanLookup($identifier, null, $e->getMessage());
        }

        return $this->renderFeeChallanLookup(
            $identifier,
            null,
            null,
            "Challan {$slip->challan_number} generated for Rs. " . number_format((float) $slip->amount_due) . '.'
        );
    }

    /** Shared render for the fee-challan lookup page — used by both the lookup form and after generating a slip. */
    private function renderFeeChallanLookup(string $identifier, ?string $notFoundError = null, ?string $slipError = null, ?string $slipSuccess = null)
    {
        $college = $this->college();
        ...
        if ($student) {
            ...
            // All challans grouped by the month of their due date, so a student
            // can see "which month's fee is this" at a glance.
            $byMonth = \App\Models\FeePayment::where('student_id', $student->id)
                ->orderByDesc('due_date')
                ->get()
                ->groupBy(fn ($p) => $p->due_date?->format('F Y') ?? 'No due date set');

            $slipCount = \App\Models\FeePayment::where('student_id', $student->id)->count();
            $invoice   = \App\Models\FeePayment::invoiceSummary($student, $feeType, $student->current_semester, $student->academic_year_id);

            $result = compact('student', 'unpaid', 'paidThisMonth', 'byMonth');
        }
        ...
        return view('public.fee-challan-download', [
            ...
            'feeType'     => $feeType,
            'slipCount'   => $slipCount,
            'slipError'   => $notFoundError ?? $slipError,
            'slipSuccess' => $slipSuccess,
        ]);
    }
