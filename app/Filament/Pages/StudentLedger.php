...
class StudentLedger extends Page
{
    ...
    /** 'full' = one slip for the whole available amount; 'installments' = split evenly. */
    public string $slipMode = 'full';
    ...
    public function generateSlip(): void
    {
        ...
        try {
            if ($this->slipMode === 'installments') {
                $slips = FeePayment::generateInstallmentPlan(
                    $student,
                    array_merge($baseData, ['remarks' => "Installment plan ({$this->slipInstallmentCount}x) generated by admin from Student Account."]),
                    $this->slipInstallmentCount,
                    $this->slipInstallmentGapDays
                );
                $this->slipSuccess = count($slips) . ' installment challans generated: ' .
                    collect($slips)->map(fn ($s) => "{$s->challan_number} (Rs. " . number_format((float) $s->amount_due) . ')')->implode(', ') . '.';
            } else {
                $slip = FeePayment::generateSlip($student, array_merge($baseData, [
                    'remarks' => 'Full-payment slip generated by admin from Student Account.',
                ]));
